...
class LongGrid(private val lines: Int, private val columns: Int, private val elements: LongArray){
    ...
    fun getRange(fromLine: Int,
        ...
        // MISSING SOME INPUT VALIDATION HERE, WHAT IF THE RANGE IS BIGGER THAN THE NEIGHBOR ELEMENTS QUANTITY?
            when(direction){
                Direction.RIGHT -> elements.copyOfRange(lines * fromLine + fromColumn, rangeSize + 1)
                // an = a1 + (n - 1) * r
                ...
                //missing this one
                Direction.DIAGONAL_UP -> (1 until rangeSize + 1).map { elements[(lines * fromLine + fromColumn) + (it - 1) * (columns + 1)] }.toLongArray()
            }
